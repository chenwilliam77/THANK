...
price1ss_price2ss=(v*L2ss*A2/L1ss/(1-v))^((1-alfa)/(alfa+vars-alfa*vars));
price1ss=[v+(1-v)*price1ss_price2ss^(vars-1)]^(-1/(1-vars));
price2ss=price1ss/price1ss_price2ss;

Rkss=exp(gam)/disc-(1-delta);
w1ss=[price1ss*alfa^alfa*(1-alfa)^(1-alfa)/(1+lambdapss)/(Rkss^alfa)]^(1/(1-alfa));
w2ss=A2*w1ss*price1ss_price2ss^(1/(alfa-1));

klr1ss=w1ss/Rkss*alfa/(1-alfa);
klr2ss=A2*klr1ss*price1ss_price2ss^(1/(alfa-1));
kss=klr1ss*L1ss+klr2ss*L2ss;
F1=lambdapss/(1+lambdapss)*L1ss*klr1ss^alfa;
y1ss=L1ss*klr1ss^alfa-F1;
yss=y1ss/v*price1ss^vars;
y2ss=(1-v)*yss*price2ss^(-vars);
% F2=lambdapss*y2ss/A2; % not needed
xss=yss;
iSss=[1-(1-delta)*exp(-gam)]*exp(gam)*kss/(1-theta);
gss=g_x*xss;
css=yss-gss-(1-theta)*iSss;

mc1ss=Rkss^alfa*w1ss^(1-alfa)/[alfa^alfa*(1-alfa)^(1-alfa)]/price1ss;
mc2ss=Rkss^alfa*(w2ss/A2)^(1-alfa)/[alfa^alfa*(1-alfa)^(1-alfa)]/price2ss;
pSss=[yss-price1ss*mc1ss*L1ss*klr1ss^alfa-price2ss*mc2ss*L2ss*klr2ss^alfa*A2^(1-alfa)]/(1-theta);
...
fun=@(x) distSS(x,w1ss,w2ss,H1ss,H2ss,tH1ss,tH2ss,tauH1ss,tauH2ss,css,gss,tss,tauss,sss,theta,fH1,fS1,gam,pss,h,disc);
...
function r=distSS(x,w1ss,w2ss,H1ss,H2ss,tH1ss,tH2ss,tauH1ss,tauH2ss,css,gss,tss,tauss,sss,theta,fH1,fS1,gam,pss,h,disc)
%global w1ss w2ss H1ss H2ss tH1ss tH2ss tauH1ss tauH2ss css gss tss tauss
%global sss theta fH1 fS1 gam pss h disc
