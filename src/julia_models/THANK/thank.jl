...
function init_parameters!(m::THANK)
    ...
    m <= parameter(:δ, 0.025, fixed=true,
                   ...
                   tex_label = "\\delta" )
    m <= parameter(:g_ss_ratio, 0.2, fixed=true,
    ...
    m <= parameter(:σ_mp, 0.2, (1e-8, 5.), (1e-8, 5.), ModelConstructors.Exponential(), RootInverseGamma(2, 0.10), fixed = false,
