...
function eqcond(m::JPT)
    ...
    Γ0[eq[:eq_ϕ], endo[:Eϕ_t]] = -m[:β] * exp(-m[:γ100]) * (1 - m[:δ])
    Γ0[eq[:eq_ϕ], endo[:z_t]] = m[:ρ_z]
    Γ0[eq[:eq_ϕ], endo[:Eλ_t]] = -1 * (1 - m[:β] * exp(-m[:γ100]) * (1 - m[:δ]))
    Γ0[eq[:eq_ϕ], endo[:ERk_t]] = -(1 - m[:β] * exp(-m[:γ100]) * (1 - m[:δ]))

    #Flexible prices
    Γ0[eq[:eq_ϕ_f], endo[:ϕ_f_t]] = 1
    Γ0[eq[:eq_ϕ_f], endo[:Eϕ_f_t]] = -m[:β] * exp(-m[:γ100]) * (1 - m[:δ])
    Γ0[eq[:eq_ϕ_f], endo[:z_t]] = m[:ρ_z]
    Γ0[eq[:eq_ϕ_f], endo[:Eλ_f_t]] = -(1 - m[:β] * exp(-m[:γ100]) * (1 - m[:δ]))
    Γ0[eq[:eq_ϕ_f], endo[:ERk_f_t]] = -(1 - m[:β] * exp(-m[:γ100]) *(1 - m[:δ]))
   ...
   Γ0[eq[:eq_kbar], endo[:μ_t]] = -(1 - (1 - m[:δ] * exp(-m[:γ100]))
   Γ0[eq[:eq_kbar], endo[:i_t]] = -(1 - (1 - m[:δ] * exp(-m[:γ100]))
   Γ1[eq[:eq_kbar], endo[:kbar_t]] = (1 - m[:δ] * exp(-m[:γ100]))
   Γ0[eq[:eq_kbar], endo[:z_t]] = (1 - m[:δ] * exp(-m[:γ100]))

   #Flexible Prices
   Γ0[eq[:eq_kbar_f], endo[:kbar_f_t]] = 1
   Γ0[eq[:eq_kbar_f], endo[:μ_t]] = -(1 - (1 - m[:δ] * exp(-m[:γ100]))
   Γ0[eq[:eq_kbar_f], endo[:i_f_t]] = -(1 - (1 - m[:δ] * exp(-m[:γ100]))
   Γ1[eq[:eq_kbar_f], endo[:kbar_f_t]] = (1 - m[:δ] * exp(-m[:γ100]))
   Γ0[eq[:eq_kbar_f], endo[:z_t]] = (1 - m[:δ] * exp(-m[:γ100]))
   ...
   Γ0[eq[:eq_wgap], endo[:b_t]] = 1 / [(1 - m[:ρ_b]) * (expγ - m[:h] * m[:β] * m[:ρ_b]) /
         * (expγ - m[:h]) / (expγ * h + expγ^2 + m[:β] * m[:h]^2)]
   ...
   Γ0[eq[:eq_R], endo[:gdp_t]] = -(1 - m[:ρ_R]) * m[:ψ1] - m[:ψ3]
   Γ0[eq[:eq_R], endo[:gdp_f_t]] = (1 - m[:ρ_R]) * m[:ψ1] + m[:ψ3]
   Γ1[eq[:eq_R], endo[:gdp_t]] = -m[:ψ3]
   Γ1[eq[:eq_R], endo[:gdp_f_t]] = m[:ψ]
